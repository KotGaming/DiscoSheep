...
public class DiscoSheepCommandExecutor implements CommandExecutor
{
	...
	private void noPermsMessage(CommandSender sender, String permission)
	{
		sender.sendMessage(ChatColor.RED
				+ "You do not have the permission node " + ChatColor.GRAY
				+ permission);
	}
			...
			String[] args)
	{
		...
		for (int i = 1; i < args.length; i++)
		{
			...
			else if (args[i].equalsIgnoreCase("-p"))
			{
				...
				if (period < DiscoParty.minPeriod
						|| period > DiscoParty.maxPeriod)
				{
					sender.sendMessage("The period in ticks must be within the range ["
		...
		if (args.length > 0)
		{
			if (args[0].equalsIgnoreCase("all"))
			{
				if (senderHasPerm(sender, PERMISSION_ALL))
				{
					...
					{
						parent.startParty(p, duration, sheepNumber, radius,
								period, fireworks);
			...
			else if (args[0].equalsIgnoreCase("me") && isPlayer)
			{
				...
				{
					parent.startParty(player, duration, sheepNumber, radius,
							period, fireworks);
					return true;
				}
				else
			...
			else if (args[0].equalsIgnoreCase("reload"))
			{
				if (senderHasPerm(sender, PERMISSION_RELOAD))
				{
					...
					sender.sendMessage(ChatColor.GREEN
							+ "DiscoSheep config reloaded from disk");
					return true;
				}
				else
				{
